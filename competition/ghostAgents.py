...
class RandomGhost( GhostAgent ):
    ...
    def getDistribution( self, state, agentIndex):
        dist = util.Counter()
        for a in state.getLegalActions( agentIndex ): dist[a] = 1.0
        dist.normalize()
        return dist
...
class DirectionalGhost( GhostAgent ):
    ...
    def getDistribution( self, state, agentIndex ):
        ...
        pacmanPosition = state.getPacmanPosition(agentIndex) # I'm not sure if this is supposed to be the ghost's index or a pacman index
